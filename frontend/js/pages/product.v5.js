import { getProduct, getProductBySlug, getProducts, addToCart, toggleWishlist, getWishlistStatus, getProductReviews, createReview, recordPurchase, hasPurchased } from '../api.js?v=11.0.0';
import { createProductCard } from '../components/productCard.v8.js?v=11.0.0';
import { createDetailSkeleton } from '../components/skeleton.js?v=11.0.0';
import { showToast } from '../components/toast.js?v=11.0.0';
import { getCurrentProductId, getCurrentProductSlug } from '../router.js?v=12.0.0';
import { isLoggedIn, getUser } from '../auth.js?v=11.0.0';
import { makeDraggable } from '../utils/dragScroll.js?v=11.0.0';
...
function renderProductUI(product) {
  ...
  // Standardized Seed (Ensures Catalog & Product Page MATCH)
  const seedString = String(product.id || product.slug || product.name || '');
  const seed = seedString.split('').reduce((a, b) => a + b.charCodeAt(0), 0);
  const pseudo = (n) => Math.abs(Math.sin(seed * n));
  const rating = product.avgRating && product.avgRating > 0 ? product.avgRating : (4.8 + (pseudo(1) * 0.2));
  const rCount = product.reviewCount && product.reviewCount > 0 ? product.reviewCount : Math.floor(pseudo(2) * 200) + 42;
      ...
      <div class="product-inner">
        ...
        <div class="product-layout">
          ...
          <div class="product-info">
            ...
            <div class="product-rating-row">
              ...
              <span class="rating-count-text">(${rCount.toLocaleString()} reviews)</span>
              <span class="views-count-text" style="display:flex;align-items:center;gap:4px;margin-left:8px;color:rgba(255,255,255,0.45);font-size:13px;"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg> ${Math.floor(740 + (product.id % 500))} views</span>
            ...
            <div class="trust-proof-card" style="margin-top:24px; padding:20px; background:rgba(124,58,237,0.05); border:1px solid rgba(124,58,237,0.15); border-radius:16px;">
              ...
              <div class="trust-gif-container" style="position:relative; width:100%; border-radius:12px; overflow:hidden; border:1px solid rgba(255,255,255,0.06);">
                <img src="/img/games/discord-proof.gif" alt="Discord Reviews Proof" style="width:100%; display:block; filter:contrast(1.05);">
        ...
        <div class="product-tabs-section">
          <div class="tabs-bar">
            ...
            <button class="tab-btn" data-tab="reviews">Reviews (${rCount.toLocaleString()})</button>
          </div>
