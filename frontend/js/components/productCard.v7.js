...
export function createProductCard(product, index = 0) {
  ...
  // Stats
  // Seeded Social Proof (Consistent but Unique per Listing)
  const seed = (String(product.id || '').split('').reduce((a, b) => a + b.charCodeAt(0), 0) + (index || 0));
  const pseudo = (n) => Math.abs(Math.sin(seed * n));
  
  const rating = (4.8 + (pseudo(1) * 0.2)).toFixed(1);
  const reviews = Math.floor(pseudo(2) * 200) + 42;
