...
function renderMobileNavbar(el) {
  ...
  if (logged) {
    actionsContent = `
      <div class="user-btn-wrap" style="position:relative">
        <button class="user-btn" id="user-btn-mobile" aria-label="Profile" aria-expanded="false" style="display:flex;">
          <div class="user-avatar" style="background:linear-gradient(135deg,#7C3AED,#22D3EE);width:32px;height:32px;font-size:13px">${initial}</div>
        </button>
    ...
    actionsContent = `
      <button id="mobile-signin-btn" class="pill-tab" style="padding: 7px 16px; font-size: 13px; background:var(--gradient-cyber); color:#fff; border:none; position:relative; pointer-events:auto !important; z-index:999999 !important;">
        SIGN IN
      </button>
    `;
  }
  ...
  el.innerHTML = `
    <nav class="navbar-inner navbar-mobile-top" style="height: 60px; border-bottom: none;">
      <a href="/index.html" class="navbar-logo" style="font-size: 18px;">
        ${hexagonSVG()}
        RAIDZONE
      </a>
      <div class="navbar-spacer"></div>
      <div class="navbar-actions" style="gap:12px">
        <button class="icon-btn" id="mobile-search-toggle" aria-label="Search">
    ...
    <div id="mobile-search-bar" class="mobile-search-wrap" style="display:none; padding: 0 16px 12px;">
      <div style="position:relative; width:100%;">
        <input type="search" placeholder="Search accounts, items..." id="mobile-search-input" 
               style="width:100%; padding:10px 16px 10px 40px; background:var(--bg3); border:1px solid var(--glass-border); border-radius:12px; color:#fff; outline:none; font-size:14px;">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="16" height="16" style="position:absolute; left:14px; top:50%; transform:translateY(-50%); color:var(--text-4)">
          <circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/>
        </svg>
      </div>
    </div>

    <!-- Mobile Game Categories Strip -->
    <div class="mobile-nav-categories" id="navbar-center-tabs">
      <!-- Injected by renderGameTabs -->
    </div>
  `;
  
  // Bind mobile search toggle
  const searchToggle = el.querySelector('#mobile-search-toggle');
  const searchBar    = el.querySelector('#mobile-search-bar');
  searchToggle?.addEventListener('click', () => {
    const isVisible = searchBar.style.display === 'block';
    searchBar.style.display = isVisible ? 'none' : 'block';
    if (!isVisible) el.querySelector('#mobile-search-input').focus();
  });
  
  // Bind mobile search input
  const searchInput = el.querySelector('#mobile-search-input');
  ...
  // Force Sign In button click (Nuclear)
  const signinBtn = el.querySelector('#mobile-signin-btn');
  if (signinBtn) {
    const goToLogin = (e) => {
      e.preventDefault();
      e.stopPropagation();
    ...
    signinBtn.addEventListener('touchstart', goToLogin, { passive: false });
  }
}
...
function injectDropdownStyles() {
  ...
  style.textContent = `
    ...
    #navbar, .navbar-inner, .navbar-cats, .navbar-left, .navbar-center, .navbar-right, .user-btn-wrap, .notif-wrap, .mobile-nav-categories { 
      overflow: visible !important; 
      pointer-events: auto !important;
    }

    .particles-container, .glow-edge-bottom {
      position: absolute;
      inset: 0;
    ...
    @media (max-width: 767px) {
      .particles-container, .glow-edge-bottom { display: none; }
    }
    ...
    /* GLOBAL MOBILE OVERRICES */
    @media (max-width: 767px) {
      body { padding-top: 104px !important; } /* Account for taller mobile header + cats */
      #navbar { 
      ...
      .brand-chip, .btn, .glass-btn, .pill-tab, .bottom-nav-item, input, select {
    ...
    #bottom-nav {
      ...
      display: flex; align-items: flex-start; z-index: 20000;
      backdrop-filter: blur(20px);
    ...
    .bottom-nav-item {
      ...
      justify-content: center; height: 64px; gap: 4px; 
    ...
    .bottom-nav-item svg { display: block; stroke: currentColor; transition: transform 0.2s ease; }
    .bottom-nav-item.active svg { transform: scale(1.1) translateY(-2px); }

    .mobile-nav-categories {
      display: flex; overflow-x: auto; scrollbar-width: none;
      padding: 0 16px 12px; gap: 8px;
    }
    .mobile-nav-categories::-webkit-scrollbar { display: none; }
    
    .mobile-nav-categories .pill-tab {
      padding: 6px 14px; font-size: 13px; flex-shrink: 0;
      background: rgba(255,255,255,0.04);
    }
    .mobile-nav-categories .pill-tab.active {
...
export function initNavbar() {
  ...
  Promise.all([
    ...
    // Clean up before re-binding to avoid duplicates
    el.querySelectorAll('[data-bound]').forEach(b => b.removeAttribute('data-bound'));
    bindDesktopEvents(el);
